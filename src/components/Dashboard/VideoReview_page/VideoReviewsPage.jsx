...
//     try {
//       const response = await axios.get('http://localhost:5000/api/video-reviews');
...
//     try {
//       const response = await axios.get(`http://localhost:5000/api/video-reviews/${id}`);
...
//     try {
//       await axios.delete(`http://localhost:5000/api/video-reviews/${id}`);
...
//     try {
//       if (selectedReview) {
//         await axios.put(`http://localhost:5000/api/video-reviews/${selectedReview}`, newReview);
//         toast.success('Video review updated');
//       } else {
//         await axios.post('http://localhost:5000/api/video-reviews', newReview);
...
//             <p className="text-gray-500 mb-2">User: {review.userId.username || 'Unknown'}</p>
...
const apiUrl =`${Base_Url}/video-reviews`
const apiUrl_users =`${Base_Url}/users`
...
const VideoReviewsPage = () => {
  ...
  const fetchUsersAndCourses = async () => {
    try {
      ...
      setUserNames(userMap);
      setCourseTitles(courseMap);
      setUsers(usersResponse.data); // Store users list for dropdown
    } catch (error) {
      console.error('Error fetching users or courses:', error);
    }
  };
  ...
  const handleSaveReview = async () => {
    try {
      if (selectedReview) {
        await axios.put(`${apiUrl}/${selectedReview}`, newReview);
        toast.success('Video review updated');
      } else {
        await axios.post(`${apiUrl}`, newReview);
        toast.success('Video review added');
      }
      fetchVideoReviews();
      setIsModalOpen(false);
